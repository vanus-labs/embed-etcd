...
const (
	...
	ZapEncodingName = "embed-etcd-log"
)
...
func init() {
	logger := logrus.New()
	logger.Formatter = &logrus.TextFormatter{TimestampFormat: timeLayout, FullTimestamp: true}
	r := &defaultLogger{
		logger: logger,
	}
	...
	vLog = r
	vLog.Debug("logger level has been set", map[string]interface{}{"log_level": level})
}
